...
void ImGui::TextUnformattedColored(const ImVec4& col, const char* text, const char* text_end) {
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            if (scale_max == scale_min) {
                col_base = GetColorU32(ImGuiCol_PlotHistogram);
                col_hovered = GetColorU32(ImGuiCol_PlotHistogramHovered);
            }
            else {
                float ratio = (v0 - scale_min) / (scale_max - scale_min);
                ImVec4 color = ImVec4(ratio < 0.5f ? 1.0f : 2.0f - 2.0f * ratio, ratio >= 0.5f ? 1.0f : 2.0f * ratio, 0.0f, 1.0f);
                col_base = ImColor(color);
                col_hovered = ImColor(ImVec4(0.2f, 0.2f, 1.0f, 1.0f));
            }
