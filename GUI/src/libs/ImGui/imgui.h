...
namespace ImGui
{
    ...
    IMGUI_API void          TextUnformattedColored(const ImVec4& col, const char* text, const char* text_end = NULL); //raw text formatting with color
    ...
    IMGUI_API void          PlotHistogramRedGreen(const char* label, const float* values, int values_count, char** values_labels, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
    ...
    IMGUI_API bool          BeginTabItemNoClose(const char* label, bool* p_open = NULL, ImGuiTabItemFlags flags = 0); // create a Tab. Returns true if the Tab is selected.
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddNgonRotated(const ImVec2& center, float radius, ImU32 col, int num_segments, float thickness = 1.0f, float alpha = 0.0f);
